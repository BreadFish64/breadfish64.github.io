...
vec4 Scaleforce(sampler2D tex, vec2 tex_coord) {
    ...
    const lfloat plus_weight = lfloat(1.0);
    const lfloat cross_weight = lfloat(1.0);
    ...
    if (total_dist == lfloat(0.0)) {
        ...
        vec2 final_offset = vec2(clamp(total_offset, -clamp_val, clamp_val)) * inv_tex_size;
